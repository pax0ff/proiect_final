...
class AppEpics extends EpicClass<AppState> {
  ...
  Stream<dynamic> call(Stream<dynamic> actions, EpicStore<AppState> store) {
    return combineEpics(<Epic<AppState>>[
      TypedEpic<AppState, ListMoviesStart>(_listMoviesStart).call,
  ...
  Stream<AppAction> _listMoviesStart(Stream<ListMoviesStart> actions, EpicStore<AppState> store) {
    return actions //
        .debounceTime(const Duration(milliseconds: 500))
        .switchMap((ListMoviesStart action) {
      return Stream<void>.value(null)
          ...
          .map((List<Movie> movies) => ListMovies.successful(movies))
          .onErrorReturnWith((Object error, StackTrace stackTrace) => ListMovies.error(error, stackTrace));
    });
  }
  ...
  Stream<AppAction> _getReviewsStart(Stream<GetReviewsStart> actions, EpicStore<AppState> store) {
    return actions //
      ...
      return Stream<void>.value(null).asyncMap((_) => api.getReviews(action.movieId)).expand((List<Review> reviews) {
      ...
      return Stream<void>.value(null)
          .asyncMap((_) {
            return api.createReview(
              movieId: store.state.selectedMovie!.id,
              text: action.text,
